...
HOST = "www.bounga.org"
REMOTE_USER = "nico"
REMOTE_DIRECTORY = "/var/www/bounga/"
...
HTML
    ...
    ratio = ((posts.length.to_f / site.posts.docs.length.to_f) * 100).to_i
...
task :deploy => [:build] do
  ...
  system("rsync -az --no-t --no-p --delete -e ssh public/ #{REMOTE_USER}@#{HOST}:#{REMOTE_DIRECTORY}")
